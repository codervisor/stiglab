...
pub fn shaping_request_to_task(req: &onsager::ShapingRequest) -> Task {
    let prompt = req
        .shaping_intent
        .get("prompt")
        .and_then(|v| v.as_str())
...
pub fn session_to_shaping_result(
    req: &onsager::ShapingRequest,
    session: &Session,
    duration_ms: u64,
) -> onsager::ShapingResult {
    let outcome = match session.state {
        SessionState::Done => onsager::factory_event::ShapingOutcome::Completed,
        SessionState::Failed => onsager::factory_event::ShapingOutcome::Failed,
        _ => onsager::factory_event::ShapingOutcome::Partial,
    };

    let content_ref = session.working_dir.as_ref().map(|dir| onsager::ContentRef {
        uri: format!("file://{dir}"),
        checksum: None,
    });
    ...
    let error = if session.state == SessionState::Failed {
        Some(onsager::protocol::ErrorDetail {
...
fn find_constraint_str(
    constraints: &[onsager::protocol::Constraint],
    name: &str,
) -> Option<String> {
    constraints
        .iter()
...
fn find_constraint_u32(constraints: &[onsager::protocol::Constraint], name: &str) -> Option<u32> {
    constraints
        .iter()
        .find(|c| c.constraint_type == name)
...
mod tests {
    ...
    fn sample_request() -> onsager::ShapingRequest {
        onsager::ShapingRequest {
            request_id: "req_001".to_string(),
            artifact_id: onsager::ArtifactId::new("art_test1234"),
            target_version: 1,
    ...
    fn test_shaping_request_to_task_minimal() {
        let req = onsager::ShapingRequest {
            request_id: "req_002".to_string(),
            artifact_id: onsager::ArtifactId::new("art_abcd1234"),
            target_version: 1,
            shaping_intent: serde_json::json!({}),
            inputs: vec![],
            constraints: vec![],
    ...
    fn test_session_to_shaping_result_done() {
        ...
        assert_eq!(
            result.outcome,
            onsager::factory_event::ShapingOutcome::Completed
        );
    ...
    fn test_session_to_shaping_result_failed() {
        ...
        assert_eq!(
            result.outcome,
            onsager::factory_event::ShapingOutcome::Failed
        );
